...
describe("Tracing Sampling Validation", () => {
  ...
  it("should have traceIdRatio sampler configured with correct parameters", () => {
    ...
    console.log(
      `TraceIdRatio sampler configured with ratio: ${sampler._ratio}`,
    );
  });
  ...
  it("should sample traces within configured rate limits", () => {
    ...
    console.log(
      `Sampling decisions: ${sampleResults.filter(Boolean).length}/${sampleResults.length} traces sampled`,
    );
  });
  ...
  it("should respect minimum sampling rate under high load", () => {
    ...
    console.log(
      `Sampling rate bounds configured: min=${minRate}, max=${maxRate}`,
    );
  });
