...
export class PortfolioService {
  ...
  /**
   * Get a summary overview for a single portfolio.
   *
   * Returns key metrics (value, asset count, allocation, rebalance settings)
   * without pulling in the full relations, making it suitable for dashboard
   * widgets or quick status checks.
   */
  ...
  /**
   * Get aggregate statistics across all of a user's active portfolios.
   *
   * Provides a single object with total value, asset count, portfolio count,
   * and per-type breakdowns that the frontend can display in a portfolio
   * overview dashboard.
   */
  async getPortfolioStats(userId: string): Promise<{
    ...
    for (const portfolio of portfolios) {
      ...
      const type = portfolio.type || PortfolioType.BALANCED;
      if (!byType[type]) {
        byType[type] = { count: 0, totalValue: 0 };
      }
      byType[type].count += 1;
      byType[type].totalValue += portfolio.totalValue || 0;
    }
    ...
    return {
      ...
      activePortfolios: portfolios.filter(
        (p) => p.status === PortfolioStatus.ACTIVE,
      ).length,
      totalValue,
      totalAssets,
      byType,
  ...
  /**
   * Export a portfolio as a JSON-serializable object.
   *
   * Includes the portfolio metadata, all assets, optimization history,
   * rebalancing events, and performance metrics in a single payload.
   */
  async exportPortfolio(portfolioId: string): Promise<{
    exportedAt: string;
    portfolio: Portfolio;
    ...
    const optimizationHistory = await this.optimizationRepository.find({
      where: { portfolioId },
      order: { createdAt: "DESC" },
    });
